...
$$F \left( x, u(x), u^{(1)}(x), \dots, u^{(k)}(x) \right) = 0,$$
where $F$ is a given function, $F: I \times  \mathbb{R}^{k+1} \to \mathbb{R}$. So solving an ODE amounts to finding a function $u$ which satisfies the relationship specified by $F$ between $u$ and its first $k$ derivatives, as well as $x$. Under certain assumptions, for such an ODE a unique solution is guaranteed to exist for given initial conditions. These initial conditions specify the value of the unknown function and its first $k-1$ derivatives at a certain initial point $x_0$, i.e. something like
$$u(x_0)=u_0, u^{(1)}(x_0)=u_1, \dots, u^{(k-1)}(x_0)=u_{k-1}.$$



existence analytical

# General Idea
 

$N_{\theta}(x)$ feed forward multilayer perceptron, i.e. maps I to R; wenn nur differentiable activation functions dann das ganze diffbar; und value der ableitung at any given point x can be computed using backpropagation; darin unterscheidet es sich von anderen ML modellen
...
vollkommen anders als in einem supervised setting wo wir für jeden x auf y wert haben; hier können wir für gegebenes x nur sagen wie sich die ableitungen von y nach x bei x zueinander verhalten

# Implementation

more specifically multilayer perceptrons,

What makes them suitable for this task is not only that they are highly flexible in approximating arbitrary functions but that they are differentiable. 

As long as they only use differentiable activation function they themselves as a composition of differentiable functions are differentiable

the latter sets them apart from other highly flexible function approximators such as random forests or boosted trees
