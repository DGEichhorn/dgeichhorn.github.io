...
description: "I implement from scratch an approach for estimating an unknown probability density function of a sample using neural networks. To demonstrate that this method works, I apply it to two examples."
...
For the purpose of mapping the data to a uniform distribution, the following loss function is a reasonable starting point: $$ L_{dist}(\theta) = \frac{1}{n} \sum_{i=1}^n \left[ N_{\theta}(x_{[i]}) -  u_{[i]}\right]^2 $$
...
3.  Repeat until convergence:
    ...
    1.  Draw $n$ i.i.d. realizations from a uniform distribution on $(0,1)$, $u_i \sim \mathcal{U}(0,1)$, and sort them in ascending order, i.e., $u_{[1]} \leq \dots \leq u_{[n]}$. $u_{[i]}$ serves as target for input $x_{[i]}$, $i=1,\dots,n$.
